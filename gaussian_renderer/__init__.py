...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, filtered_mask = None):
    ...
    try:
        ...
        # By default, only the gradients of leaf tensors (tensors that don't require gradient calculation for other tensors) are computed during backpropagation.
        # retain_grad allows you to override this behavior for non-leaf tensors.

            # import torch
            # x = torch.tensor([2.0], requires_grad=True)
            # y = x * x  # y is a non-leaf tensor
            # 
            # # Gradient of y will not be computed by default
            # y.backward()
            # 
            # # To access the gradient of y
            # print(y.grad)  # Prints None
            # 
            # # Use retain_grad to enable gradient calculation for y
            # y.retain_grad()
            # y.backward()
            # 
            # # Now you can access the gradient
            # print(y.grad)  # Prints torch.tensor([4.])
    ...
    # theoretically, screen_width = distance_to_object * tanfovx * 2
    ...
    if filtered_mask is not None:
        ...
        # Unlike assignment (=), which modifies the original tensor in-place, 
        # clone() returns a completely new tensor object with the same data as the original. 
        # This new tensor has its own memory allocation, independent of the original.
