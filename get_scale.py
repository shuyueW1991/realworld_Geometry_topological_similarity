...
def get_combined_args(parser : ArgumentParser):
    """
    A function that combines arguments from the command line and a config file, and returns a Namespace object.
    """
    ...
    return Namespace(**merged_dict)  # all the args awating `extract` elongs to type Namespace, like the ones in train_scene.py that goes through parser.parse_args b.
...
if __name__ == '__main__':
    ...
    # assert os.path.exists(os.path.join(dataset.source_path, 'images_8')) and "Please specify a valid image root."
    ...
    # for i, image_path in tqdm(enumerate(sorted(os.listdir(os.path.join(dataset.source_path, 'images_8'))))):
        # print(image_path)
        image = cv2.imread(os.path.join(os.path.join(dataset.source_path, 'images'), image_path))
        # image = cv2.imread(os.path.join(os.path.join(dataset.source_path, 'images_8'), image_path))
